...
import BotonStandard from "./components/BotonStandard";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const usuario = response.data.usuario; // Asegúrate de que la respuesta contenga los datos del usuario
        console.log("Datos del usuario recibidos:", usuario);

        // Guardar la información del usuario en localStorage
        localStorage.setItem("usuario", JSON.stringify(usuario));

        // Solicitar los tokens después de login (nueva solicitud)
        const tokensResponse = await axios.get(
          `http://localhost:5000/api/usuario/${usuario.id}/tokens`
        );
        if (tokensResponse.status === 200) {
          const tokens = tokensResponse.data.tokens; // Asumimos que la respuesta tiene los tokens
          // Guardar los tokens en localStorage también
          const usuarioData = { ...usuario, tokens };
          localStorage.setItem("usuario", JSON.stringify(usuarioData));
        }

        // Redirigir después de 3 segundos (3000ms)
        setTimeout(() => {
          navigate("/dashuser"); // Redirige a la página de dashuser
    ...
    <div className="bg-[#ff6c17] h-[90vh] flex items-center justify-center">
      ...
      <div className="bg-gray-900 opacity-90 h-[80%] w-[50%] flex flex-col items-center justify-center rounded-[2rem] border-2 border-[#ff6c17] shadow-lg shadow-black z-20">
        ...
        <div>
          ...
          <BotonStandard className="font-extrabold" to="/registrate">
            Registrate
          </BotonStandard>
        </div>
